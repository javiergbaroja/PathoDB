...
def arabic_to_roman(n: int) -> str:
    """Convert a small integer to an uppercase Roman numeral string."""
...
def to_roman_if_pre2011(b_case_val, probe_val):
    if extract_year(b_case_val) < 2011:
        try:
            return arabic_to_roman(int(probe_val))
...
def load_blocks(filepath: str, conn, submission_map: dict, dry_run: bool) -> tuple:
    ...
    Pre-2011 exports have no Probe column. Those rows receive a sentinel
    lis_probe_id of "1" so they participate in the same lookup logic as
    modern rows. Any pre-2011 submission that is absent from blocks.csv
    entirely also gets a sentinel probe inserted (Pass 1b), ensuring that
    scans can always resolve to it via try_submission_match.
    ...
    for _, prow in tqdm(probes_df.iterrows(), total=len(probes_df), desc="  Probes"):
        ...
        # FIX 1: Pre-2011 exports have no Probe value — assign sentinel "1"
        # so the row is not silently discarded and can be resolved by scans.
        if not lis_probe_id and lis_sub_id and extract_year(lis_sub_id) <= 2011:
            lis_probe_id = "1"

        if not lis_probe_id or not lis_sub_id:
            stats["warnings"].append(
                f"Probe row missing Probe or Einsendung — skipped: {prow.to_dict()}"
            )
            continue
    ...
    # ── Pass 1b: sentinel probes for pre-2011 submissions absent from blocks.csv
    # A pre-2011 submission that has no rows in blocks.csv at all will never
    # appear in probe_map, so scans that reference it via try_submission_match
    # find sub_to_probes empty and fail. We insert a bare sentinel probe here
    # so those scans can auto-resolve.
    sentinel_count = 0
    for lis_sub_id, sub_id in submission_map.items():
        if extract_year(lis_sub_id) > 2011:
            continue
        if any(sk == lis_sub_id for (sk, _) in probe_map):
            continue  # already has at least one probe from Pass 1
        ...
        if not dry_run:
            ...
            probe_id = -abs(hash((lis_sub_id, "1")))

        probe_map[(lis_sub_id, "1")] = probe_id
        block_map[probe_id] = {}
    ...
    if sentinel_count:
        log.info(
            f"  Pass 1b: {sentinel_count} sentinel probe(s) created for pre-2011 "
            f"submissions absent from blocks.csv"
        )
    ...
    for idx, brow in tqdm(df.iterrows(), total=len(df), desc="  Blocks"):
        ...
        # Mirror Fix 1 for block rows so they look up the same sentinel key
        if not lis_probe_id and lis_sub_id and extract_year(lis_sub_id) <= 2011:
            lis_probe_id = "1"
...
def load_scans(
    ...
    Era logic (based on year in b_case):
      ≤ 2011 : b_case = submission ID; empty probe filled with sentinel "1"
      2011-17: b_case = probe lis_probe_id directly
      ≥ 2017 : b_case + zero-padded probe numeral = composite lis_probe_id;
               empty probe filled with sentinel "1"
    """
    df = read_csv(filepath)

    # FIX 3: Fill empty probe with sentinel "1" for both era extremes.
    # Post-2017: single-probe submissions omit the probe column entirely.
    # Pre-2011:  the probe column did not exist in PathoWin exports.
    ...
    df.loc[na_or_empty_probe_mask & (df["b_case"].apply(extract_year) <= 2011), "probe"] = "1"

    # b_case can be 'B2025.00042'; strip leading zeros after the dot so it
    # matches the lis_probe_id stored in probe_map (e.g. 'B2025.42').
    df["b_case"] = df["b_case"].str.replace(r'\.(0+)(\d+)$', r'.\2', regex=True)
    df["probe"] = df.apply(
        lambda r: to_roman_if_pre2011(r["b_case"], r["probe"]), axis=1
    )
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="  Scans"):
        ...
        if stain_raw == "":
            stats["warnings"].append(
                f"Row {idx}: empty stain value treated as missing — added as 'unmatched'"
            )
            stain_raw = "unmatched"
        ...
        if probe_id is None:
            stats["warnings"].append(
                f"Row {idx}: submission {b_case!r} found but probe could not be resolved for {filename}"
