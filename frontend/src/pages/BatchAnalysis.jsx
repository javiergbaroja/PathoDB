...
export default function BatchAnalysis() {
  ...
  const [onePerBlock, setOnePerBlock]         = useState(false) // NEW: Toggle state
  ...
  const matchMutation = useMutation({
    ...
    onSuccess: (data) => {
      const matched = data.matched.map(r => {
        ...
        return { ...r, stain_category: stainCategory, block_id: r.block_id }
      })
  ...
  const loadCohortMutation = useMutation({
    ...
    onSuccess: (data) => {
      const matched = data.results.map(r => {
        ...
        return {
          scan_id: r.scan_id,
          block_id: r.block_id, // NEW: Capture block_id
  ...
  // 1. Initial filter by stain category checkboxes
  let filteredMatched = matchResults?.matched.filter(m => selectedStains.includes(m.stain_category)) || []
  
  // 2. Group & keep only one per block (if toggle is active)
  if (onePerBlock && filteredMatched.length > 0) {
    const blockMap = new Map()
    filteredMatched.forEach(m => {
      // Manual pastes that bypass the block mapping fallback to scan_id to avoid accidental merging
    ...
    filteredMatched = Array.from(blockMap.values()).map(slides => {
      // Prioritize a slide that matches the model's required stains explicitly
          ...
          <div style={{ background: 'white', borderRadius: 10, padding: 24, border: '1px solid var(--border-l)', boxShadow: 'var(--shadow-s)', animation: 'fadeIn 0.3s' }}>
            ...
            <div style={{ marginBottom: 20, padding: 16, background: 'var(--navy-05)', borderRadius: 8, border: '1px solid var(--border-l)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy)' }}>Filter by Stain Category</div>
