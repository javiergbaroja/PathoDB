...
export default function BatchAIModal({ isOpen, onClose, projectId, projectClasses = [], projectScans: initialScans }) {
  ...
  // Feature-extraction models produce no GeoJSON — skip all annotation-import UI.
  const isFeatureExtraction = selectedModel?.result_type === 'feature_extraction'

  // 3. Auto-Match Logic (Only for annotation-producing models)
  useEffect(() => {
    if (selectedModel && !isFeatureExtraction && importMode === 'map_classes' && projectClasses.length > 0) {
      const initialMap = {}
  ...
  }, [selectedModel, importMode, projectClasses, isFeatureExtraction])
  ...
  const handleSubmit = () => {
    ...
    // Class-mapping validation only applies to annotation-producing models.
    if (!isFeatureExtraction && importMode === 'map_classes' && Object.values(classMapping).some(v => v === '')) {
      setErrorMsg('Please map all classes or set them to "Ignore".'); return;
    }

    const params = isFeatureExtraction
      ? { project_id: projectId }  // no import_mode / class_mapping for feature extraction
      : {
    ...
    submitMutation.mutate({
      ...
      output_directory: '',  // empty string → auto-ingest path handled by API
      scan_ids:         scans.map(s => s.scan_id || s.id),
      params,
    })
  }

  const mappingIncomplete = !isFeatureExtraction
    && importMode === 'map_classes'
    && Object.values(classMapping).some(v => v === '')

  const canSubmit = !!selectedModelId && !mappingIncomplete && !isMetadataLoading && !modelsLoading

  // Step numbering: feature-extraction models skip steps 2 & 3.
  const step2Label = isFeatureExtraction ? null : '2. Import Strategy'
  const step3Label = isFeatureExtraction ? null : '3. Class Mapping'
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {successMsg && (
          <div style={{ color: 'var(--success)', background: 'var(--success-bg)', padding: 12, borderRadius: 6, fontSize: 13, border: '1px solid var(--teal-light)' }}>
            {successMsg}
          </div>
        )}
        ...
        {(isMetadataLoading || modelsLoading) ? (
          ...
          <>
            ...
            <div>
              ...
              {/* Feature-extraction callout — no annotations will be imported */}
              {isFeatureExtraction && (
                <div style={{
                  marginTop: 10,
                  padding: '10px 12px',
                  ...
                  <strong>Feature Extraction mode</strong> — this model writes <code>.npz</code> files to the output directory and does not produce slide annotations. The class-mapping and import-strategy steps do not apply.
                </div>
              )}
            </div>

            {/* ── Step 2: Import Strategy (annotation models only) ─────────── */}
            {selectedModel && !isFeatureExtraction && (
            ...
            {/* ── Step 3: Class Mapping Table (annotation + map_classes only) ─ */}
            {selectedModel && !isFeatureExtraction && importMode === 'map_classes' && (
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--navy)', marginBottom: 8, textTransform: 'uppercase' }}>
                  {step3Label}
                </label>
                <div style={{ border: '1px solid var(--border-l)', borderRadius: 8, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 20px 1fr', background: 'var(--navy-05)', padding: '8px 12px', fontSize: 11, fontWeight: 600, color: 'var(--text-3)', borderBottom: '1px solid var(--border-l)' }}>
                  ...
                  <div style={{ maxHeight: 200, overflowY: 'auto', background: 'white' }}>
                    {(() => {
                      ...
                      // Sort: auto-matched classes first, then alphabetically
                      ...
                      return sortedClasses.map(mc => {
                        ...
                        return (
                          <div key={mcName} style={{ display: 'grid', gridTemplateColumns: '1fr 20px 1fr', gap: 12, padding: '10px 12px', borderBottom: '1px solid var(--border-l)', alignItems: 'center' }}>
                            ...
                            <div>
                              <select
                                ...
                                style={{
                                  ...
                                  border: currentValue === '' ? '1px solid var(--danger)'
                                    : isAutoMatched    ? '1px solid var(--success)'
                                    :                    '1px solid var(--border-l)',
                                  background: currentValue === '' ? 'var(--danger-bg)'
                                    : isAutoMatched ? 'var(--success-bg)'
                                    :                'white',
                                }}
                              >
