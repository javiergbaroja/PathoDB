...
import { toSVGPath, projectPoints, translateGeometry, applyRectHandle, applyEllipseHandle, dist, VERTEX_R, FIRST_VERTEX_R, CLOSE_THRESH, RECT_CURSOR, ELLIPSE_CURSOR, hitVertexHandle, hitRectHandle, hitEllipseHandle, hitTestBody } from '../../lib/annotationMath'
import { useSelectTool, useBrushTool, usePolygonTool, useShapeTool } from './hooks/useAnnotationTools'
import { AnnotationShape, LiveAnnotation, RectHandleOverlay, EllipseHandleOverlay, PolygonHandleOverlay } from './AnnotationShapes'
...
export default function AnnotationLayer({
  ...
  // FIX 3: Fully strict replacement routing
  function onEmit(ann) {
    console.log("🚀 EMITTING TO BACKEND:", ann.geometry.points) // <-- ADD THIS
    if (readOnly) return
    if (ann._replaceId) {
      if (onAnnotationUpdated) {
        onAnnotationUpdated(ann._replaceId, ann.geometry)
      }
      return 
    }
  ...
  function onPointerUp(e) {
    ...
    if (activeTool === 'select') select.onPointerUp(e, mouse, suppressClickRef, pendingDeselectRef)
  ...
  function onPointerCancel() { select.cancel(); brush.cancel(); poly.cancel(); shape.cancel(); pendingDeselectRef.current = false; suppressClickRef.current = false }

  function onClick(e) {
    if (isSpacePanRef.current || isMiddlePanRef.current) return
    if (suppressClickRef.current) { suppressClickRef.current = false; return }
    if (!activeTool || readOnly) return
    const el = getEl(e), img = elementToImage(osdRef.current, el.x, el.y)
  ...
  const dragAnn = select.activeDrag?.annId ? annotations.find(a => a.id === select.activeDrag.annId) ?? null : null
  const selAnn = (activeTool === 'select' && selectedAnnId) ? annotations.find(a => a.id === selectedAnnId) ?? null : null
  ...
  if (select.activeDrag && curImg) {
    ...
    if (k === 'move' && select.activeDrag.startImg) {
      const dx = curImg.x - select.activeDrag.startImg.x, dy = curImg.y - select.activeDrag.startImg.y
  ...
  const overlayGeom = (liveGeom && (select.activeDrag?.kind === 'rectHandle' || select.activeDrag?.kind === 'ellipseHandle')) ? liveGeom : selAnn?.geometry
  const overlayVtxPts = liveVtxPts ?? ((selAnn?.annotation_type === 'polygon' || selAnn?.annotation_type === 'brush') ? (selAnn.geometry?.points ?? []) : null)
  const showHandles = activeTool === 'select' && selAnn && !readOnly && select.activeDrag?.kind !== 'move'

  const projPolyPts = poly.polyPts.map(p => imageToElement(viewer, p.x, p.y)).filter(Boolean)
  const nearFirst = activeTool === 'polygon' && projPolyPts.length >= 3 && mouse && dist(mouse, projPolyPts[0] ?? { x: -999, y: -999 }) <= CLOSE_THRESH
  ...
  const viewRadius = () => {
    ...
    const o = imageToElement(viewer, 0, 0), f = imageToElement(viewer, brushRadius * brush.pressureRef.current, 0)
  ...
  const dynamicCursor = (() => {
    ...
    if (activeTool === 'select') {
      ...
      for (let i = annotations.length - 1; i >= 0; i--) {
        if (hitTestBody(viewer, annotations[i], mouse)) return annotations[i].id === selectedAnnId ? (readOnly ? 'pointer' : 'move') : 'pointer'
      }
      return 'default'
    }
    return 'crosshair'
  ...
  return (
    <svg ref={svgRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: activeTool ? 'all' : 'none', cursor: dynamicCursor, zIndex: 50 }}
      onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp}
      onPointerLeave={onPointerLeave} onPointerCancel={onPointerCancel} onClick={onClick}
      ...
      {liveGeom && liveType && <LiveAnnotation viewer={viewer} type={liveType} geometry={liveGeom} color={liveColor} />}
      {liveVtxPts && dragAnn && <LiveAnnotation viewer={viewer} type={dragAnn.annotation_type} geometry={{ points: liveVtxPts }} color={dragAnn._color || '#6ee7b7'} />}

      {showHandles && (
        <>
          {['polygon', 'brush'].includes(selAnn.annotation_type) && overlayVtxPts && <PolygonHandleOverlay viewer={viewer} pts={overlayVtxPts} color={selAnn._color || '#6ee7b7'} />}
          {selAnn.annotation_type === 'rectangle' && overlayGeom && <RectHandleOverlay viewer={viewer} g={overlayGeom} color={selAnn._color || '#6ee7b7'} />}
          {selAnn.annotation_type === 'ellipse' && overlayGeom && <EllipseHandleOverlay viewer={viewer} g={overlayGeom} color={selAnn._color || '#6ee7b7'} />}
        </>
      )}

      {projBrushROI && (
        <path d={toSVGPath(projBrushROI, true)} fillRule="evenodd" fill={brush.subtractMode.current ? 'rgba(230,0,46,0.15)' : toolColor + '33'}
          stroke={brush.subtractMode.current ? 'rgba(230,0,46,0.8)' : toolColor} strokeWidth={1.5} strokeLinejoin="round" style={{ pointerEvents: 'none' }} />
      )}

      {activeTool === 'polygon' && projPolyPts.length > 0 && (
        <g>
          {projPolyPts.length >= 3 && <path d={toSVGPath(projPolyPts, true)} fill={toolColor} fillOpacity={0.25} stroke="none" />}
          <path d={toSVGPath(projPolyPts, false)} fill="none" stroke={toolColor} strokeWidth={1.5} strokeLinejoin="round" />
          {mouse && (
            <>
              <line x1={projPolyPts[projPolyPts.length - 1].x} y1={projPolyPts[projPolyPts.length - 1].y} x2={mouse.x} y2={mouse.y} stroke={toolColor} strokeWidth={1.5} strokeDasharray="6 3" opacity={0.85} />
              {projPolyPts.length >= 2 && <line x1={mouse.x} y1={mouse.y} x2={projPolyPts[0].x} y2={projPolyPts[0].y} stroke={toolColor} strokeWidth={1} strokeDasharray="2 6" opacity={0.35} />}
            </>
          )}
          {projPolyPts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r={i === 0 ? FIRST_VERTEX_R : VERTEX_R} fill={i === 0 ? '#ff7c00' : toolColor} stroke="white" strokeWidth={1.5} />)}
          {poly.polyFreehandRef.current && projPolyPts.length > 2 && <text x={projPolyPts[projPolyPts.length - 1].x + 10} y={projPolyPts[projPolyPts.length - 1].y - 10} fill={toolColor} fontSize={10} fontFamily="monospace" style={{ pointerEvents: 'none', userSelect: 'none' }}>freehand</text>}
        </g>
      )}
      ...
      {activeTool === 'point' && mouse && <g style={{ pointerEvents: 'none' }}><circle cx={mouse.x} cy={mouse.y} r={8} fill={toolColor} fillOpacity={0.3} stroke={toolColor} strokeWidth={1.5} /><circle cx={mouse.x} cy={mouse.y} r={2} fill={toolColor} /></g>}
      
      {(mouse && activeTool === 'brush') && <BrushLimits cx={mouse.x} cy={mouse.y} radius={viewRadius()} subtract={brush.subtractMode.current} visible={brush.brushLimitsV} />}
    </svg>
  )
}