...
export default function SlideViewer() {
  ...
  async function fetchAndRenderOverlay(jobId, file, viewer) {
    const key = `${jobId}:${file}`
    try {
      const geojson   = await api.getAnalysisOverlay(jobId, file)
      const info      = leftInfo
      const slW       = info?.width
      const slH       = info?.height
      if (!slW || !slH || !viewer || !geojson?.features?.length) return

      // Colour per QuPath classification name
      const CAT_COLORS = {
        'Lymph node':  '#e56454',
        'Metastasis':  '#75ad51', 
        'Deposit':     '#d4b93c',
      ...
      function categoryColor(name) {
        ...
        // Fall back: decode QuPath signed int if present
        return '#94a3b8'
      }
      ...
      geojson.features.forEach(feature => {
        ...
        const name  = feature.properties?.classification?.name || 'other'
        const color = categoryColor(name)
        
        // Normalize everything into an array of Polygons.
        // A Polygon is an array of rings [exterior, hole1, hole2...]
        ...
        polygons.forEach(poly => {
          ...
          // Loop through the exterior ring and all interior holes
          poly.forEach(ring => {
            ring.forEach(([x, y], i) => {
              const localX = ( x ) / slW
              const localY = ( y ) / slW
              
              // M = Move to (start), L = Line to (continue)
              pathData += i === 0 ? `M ${localX},${localY} ` : `L ${localX},${localY} `
            })
            pathData += 'Z ' // Z = Close the current ring
          })

          // Draw the entire Polygon (with all its holes) as a SINGLE path
          ...
          // CRITICAL: This is the magic that actually subtracts the holes
      ...
      // Attach to OSD viewer overlay system
      ...
      // Cache geojson so toggling hide→show doesn't re-fetch
      setOverlayData(prev => ({ ...prev, [key]: geojson }))
  ...
  function clearOverlay(jobId, viewer) {
    const refs = overlayRefs.current[jobId] || []
    refs.forEach(({ svg, viewer: v }) => {
      try { v.removeOverlay(svg) } catch (_) {}
    })
    overlayRefs.current[jobId] = []
  }

  async function handleToggleOverlay(jobId, file = 'metastasis') {
    ...
    if (isOn) {
      ...
      // Render all files for this model (metastasis + ln for lvi_detector)
      // Determine which files exist from result
      await fetchAndRenderOverlay(jobId, 'metastasis', viewer)
      await fetchAndRenderOverlay(jobId, 'ln',         viewer)
  ...
  return (
    <div style={{ width: 296, flexShrink: 0, background: 'rgba(2,5,18,0.98)', borderLeft: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '9px 12px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)' }}>H100 · gpu-invest</span>
      </div>
