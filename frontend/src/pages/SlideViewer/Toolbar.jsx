...
export default function Toolbar({ handleBack, leftInfo, rightInfo, compareMode, leftZoom, rightZoom, handleCompareToggle }) {
  ...
  // Ruler and polygon tool are mutually exclusive — handle in the click handlers
      ...
      <div style={{ height: 48, flexShrink: 0, background: 'rgba(3,8,25,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10, padding: '0 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
          ...
          <button
            ...
            style={isPolygonActive ? { borderColor: '#ffd700', color: '#ffd700', background: 'rgba(255,215,0,0.12)' } : {}}
          >
          ...
          {/* ── Live ROI indicator + clear button ─────────────────────────── */}
          {polygons.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(255,215,0,0.1)', border: '1px solid rgba(255,215,0,0.35)', borderRadius: 5, padding: '3px 8px' }}>
...
function ShortcutsOverlay({ onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'rgba(3,8,25,0.98)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '18px 22px', minWidth: 240 }}>
