...
// Shared OpenSeadragon initialisation hook.
// Used by both SlideViewer (diagnostic) and ProjectDetail (annotation).
...
/**
 * @param {object} opts
 * @param {React.RefObject} opts.containerRef  - div ref for the OSD canvas
 * @param {number|null}     opts.scanId        - current scan ID
 * @param {object|null}     opts.slideInfo     - slide info from API (mpp_x etc.)
 * @param {string}          opts.token         - JWT for tile URLs
 * @param {function}        opts.onZoom        - called with (zoom) on viewport change
 * @param {React.RefObject} opts.osdRef        - mutable ref to OSD Viewer instance
 * @returns {{ destroy: function }}
 */
export function useOSDViewer({ containerRef, scanId, slideInfo, token, onZoom, osdRef }) {
  ...
  const init = useCallback(() => {
    ...
    fetch(`/api/slides/${scanId}/dzi?token=${token}`)
      ...
      .catch(e => console.error(`OSD init failed: ${e.message}`))
  }, [scanId, slideInfo, token]) // eslint-disable-line
  ...
  useEffect(() => {
    ...
    const loadOSD = (cb) => {
      ...
      if (window.OpenSeadragon) {
        ...
        s.onload = cb; document.head.appendChild(s); return
      }
...
/**
 * Convert viewer-element pixel coords → image pixel coords.
 */
...
/**
 * Convert image pixel coords → viewer-element pixel coords.
 */
